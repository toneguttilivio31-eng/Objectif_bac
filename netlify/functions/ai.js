...
const GEMINI_MODEL_FALLBACKS = ['gemini-flash-latest','gemini-3.5-flash'];
...
exports.handler=async(event)=>{
  ...
  try{
    ...
    // OpenRouter is the primary provider. Gemini is the automatic fallback,
    // including for 429/rate-limit, 5xx and other provider failures.
    ...
    if(!r.ok){
      if(geminiKey){
        ...
        for(const im of images){
          ...
          const data64=String(im.data);
          if(data64.length>6500000) return json(413,{error:'Une photo est trop lourde. Choisis une photo plus légère.',stage:'input'});
          parts.push({inlineData:{mimeType:mime,data:data64}});
        }
        const contents=[{role:'user',parts}];
        const gr=await callGeminiWithFallback({
          key:geminiKey, model:geminiModel, contents, system:systemText,
          maxOutputTokens:Math.min(Math.max(Number(b.maxTokens)||4200,800),6000)
        });
        if(gr.response.ok && gr.answer){
          return json(200,{text:gr.answer,provider:'gemini',model:gr.data?.modelVersion||gr.model, fallbackFrom:'openrouter'});
        }
      }
      return json(r.status>=400?r.status:502,{
        error:getOpenRouterError(data,r.status),
        stage:'openrouter',
        httpStatus:r.status,
        model,
